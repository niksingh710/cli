...
```
bash <(curl -s https://raw.githubusercontent.com/niksingh710/cli/master/setup)
```
...
- Arch Specific
  ...
  ```
  yay -S rustup python python-pip fnm-bin go tmux tmux-bash-completion-git zsh-completion-git bash-complete-alias bash-completion fd ripgrep shellcheck-bin shfmt advcpmv pistol-git bat exa git glow urlview git-secret trash-cli
  ```
